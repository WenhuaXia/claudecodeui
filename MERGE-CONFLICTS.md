...
# ✅ Git 报告无冲突，但 zh-CN/sidebar.json 静默丢失了 4 个 key：
#   archiveSessionNotice, archivedSessionNotice, archiveSession, deleteSessionPermanently
# 根因：ours 覆盖了分支新增的 deleteConfirmation 子字段
# 解决：从 fix/tasks-i18n-small 恢复完整 sidebar.json 的 deleteConfirmation 段
# 后续重建时必须检查此文件！
...
# ⚠️ 静默冲突！merge 后 claude-session-synchronizer.provider.ts 从 493 行变成 201 行
# 根因：git 自动用 ours 覆盖了分支新增的 292 行（generateAiTitle 等函数全部丢失）
# 解决：merge 后手动从分支恢复完整文件，commit fix
# 后续重建时必须再次手动覆盖此文件！（见下方"必须手动覆盖的文件"）
...
| **claude-session-synchronizer.provider.ts** | **theirs** | ⚠️ auto-rename 分支新增 292 行（generateAiTitle 等），ours 会静默丢失 |
| i18n 翻译冲突（settings.json） | **theirs** | 新增翻译需要保留 |
| i18n 翻译（sidebar.json 等嵌套 JSON） | **theirs** | ⚠️ Git 合并会保留顶层结构但丢失分支新增的子字段（如 deleteConfirmation 下新增的 4 个 key） |
