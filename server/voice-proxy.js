...
function isAllowedBackendUrl(raw) {
    ...
    if (u.protocol !== 'http:' && u.protocol !== 'https:')
        return false;
    if (u.hostname === '169.254.169.254' || u.hostname.startsWith('169.254.'))
        return false;
    return true;
}
/**
...
router.post('/transcribe', async (req, res) => {
    ...
    if (!cfg.baseUrl)
        return res.status(503).json({ error: 'No voice backend configured' });
    ...
    upload.single('audio')(req, res, async (err) => {
        ...
        try {
            ...
            fd.append('file', new Blob([req.file.buffer], { type: req.file.mimetype || 'audio/webm' }), req.file.originalname || 'recording.webm');
...
//# sourceMappingURL=voice-proxy.js.map