...
test('providerSkillsService lists claude user, project, and enabled plugin skills', { concurrency: false }, async () => {
    ...
    try {
        ...
        await writeSkill(path.join(commandPluginInstallPath, 'skills'), 'ignored-command-plugin-skill-dir', 'ignored-command-plugin-skill', 'Command plugin fallback skill should be ignored');
...
test('providerSkillsService lists gemini and cursor skills from their configured directories', { concurrency: false }, async () => {
    ...
    const restoreHomeDir = patchHomeDir(tempRoot);
    try {
        await writeSkill(path.join(tempRoot, '.gemini', 'skills'), 'gemini-user-dir', 'gemini-user', 'Gemini user skill');
        await writeSkill(path.join(tempRoot, '.agents', 'skills'), 'agents-user-dir', 'agents-user', 'Agents user skill');
        await writeSkill(path.join(workspacePath, '.gemini', 'skills'), 'gemini-project-dir', 'gemini-project', 'Gemini project skill');
...
test('providerSkillsService adds global skills for claude, codex, gemini, and cursor', { concurrency: false }, async () => {
    ...
    try {
        ...
        assert.equal(createdGeminiSkill.sourcePath.endsWith(path.join('.gemini', 'skills', 'gemini-global-dir', 'SKILL.md')), true);
