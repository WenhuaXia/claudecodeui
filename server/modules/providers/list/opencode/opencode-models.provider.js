...
import { buildDefaultProviderCurrentActiveModel, getOpenCodeDatabasePath, readObjectRecord, readOptionalString, writeProviderSessionActiveModelChange, } from '../../../../shared/utils.js';
...
export const parseOpenCodeModelsStdout = (stdout) => {
    const ids = [];
...
const formatDateToken = (token) => (`${token.slice(0, 4)}-${token.slice(4, 6)}-${token.slice(6, 8)}`);
const formatModelToken = (token, nextToken) => {
...
const formatOpenCodeModelSlug = (slug) => {
    const labelParts = [];
    const dateParts = [];
...
const readOpenCodeModelParts = (id) => {
...
const labelForOpenCodeModelId = (id) => {
...
const runOpenCodeModelsCommand = () => new Promise((resolve, reject) => {
