...
export class ClaudeSessionSynchronizer implements IProviderSessionSynchronizer {
    ...
    for (const block of data?.content || []) {
      if (block?.type === 'thinking' && typeof block.thinking === 'string') {
        ...
        for (const pattern of decisionPatterns) {
          ...
          if (candidate && !this.isPromptMatch(candidate, userPrompt) && !this.isLanguageMismatch(candidate, userPrompt)) return candidate;
        }
        ...
        const validBackticks = allBackticks.filter((m) => !this.isPromptMatch(m[1], userPrompt) && !this.isLanguageMismatch(m[1], userPrompt));
        ...
        const validQuotes = allQuotes.filter((m) => !this.isPromptMatch(m[1], userPrompt) && !this.isLanguageMismatch(m[1], userPrompt));
  ...
  /**
   * Reject candidates that are in a different language than the user's message.
   * Reasoning models produce English internal monologue in thinking blocks.
   * If the user speaks Chinese, an English candidate like "is what the user said"
   * is clearly AI internal reasoning leaked, not a valid title.
   */
  private isLanguageMismatch(candidate: string, userPrompt: string): boolean {
    const userHasCJK = /[\u3000-\u9FFF\uF900-\uFAFF]/.test(userPrompt);
    if (!userHasCJK) return false;
    const candidateHasCJK = /[\u3000-\u9FFF\uF900-\uFAFF]/.test(candidate);
    return !candidateHasCJK;
  }
