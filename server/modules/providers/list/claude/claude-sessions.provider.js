...
async function getSessionMessages(sessionId, providerSessionId, limit, offset) {
    try {
        ...
        const sortedMessages = messages.sort((a, b) => new Date(a.timestamp || 0).getTime() - new Date(b.timestamp || 0).getTime());
 ...
 */
function isContextResumptionSummary(raw) {
...
function isSubagentPromptEcho(text, subagentPrompts) {
    if (!subagentPrompts)
        return false;
...
export class ClaudeSessionsProvider {
     ...
     */
    normalizeMessage(rawMessage, sessionId, subagentPrompts = null) {
         ...
         */
        const isHumanOrigin = !raw.isSynthetic
