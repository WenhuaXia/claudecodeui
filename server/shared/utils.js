 ...
 */
export function getProviderSessionActiveModelChangesPath() {
    return path.join(os.homedir(), '.cloudcli', 'provider-session-active-model-changes.json');
}
const buildProviderSessionActiveModelChangeKey = (provider, sessionId) => `${provider}:${sessionId}`;
const isProviderSessionActiveModelChangeCacheEntry = (value) => {
    const record = readObjectRecord(value);
...
const readProviderSessionActiveModelChangeCacheFile = async (filePath) => {
    try {
        ...
        const entries = Object.fromEntries(Object.entries(parsed.entries).filter((entry) => isProviderSessionActiveModelChangeCacheEntry(entry[1])));
        return {
            version: PROVIDER_SESSION_ACTIVE_MODEL_CHANGE_CACHE_VERSION,
            entries,
        };
...
const writeProviderSessionActiveModelChangeCacheFile = async (filePath, payload) => {
    await mkdir(path.dirname(filePath), { recursive: true });
    await writeFile(filePath, `${JSON.stringify(payload, null, 2)}\n`, 'utf8');
};
const buildUnsupportedProviderSessionActiveModelChange = (provider, sessionId) => ({
    provider,
    sessionId,
    supported: false,
    changed: false,
 ...
 */
export async function readProviderSessionActiveModelChange(provider, sessionId, options = {}) {
    ...
    const cacheEntry = cacheFile.entries[buildProviderSessionActiveModelChangeKey(provider, normalizedSessionId)];
 ...
 */
export async function writeProviderSessionActiveModelChange(provider, input, options = {}) {
